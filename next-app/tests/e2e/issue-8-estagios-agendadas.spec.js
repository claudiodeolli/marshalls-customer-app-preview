...
test('Cada estágio traz o ícone que lhe corresponde', async ({ page }) => {
  // O mapeamento é o mesmo desde o PDF — calendário enquanto conta dias,
  // relógio de 23h para baixo. O que mudou na #11 foi o veículo: os ícones
  // deixaram de ser caracteres unicode e viraram assets locais do Fluent
  // Emoji 3D, para não mudarem de desenho conforme o sistema.
  ...
  for (const { emDias, src } of icones) {
    expect(src, emDias ? 'contagem em dias' : 'contagem em horas/minutos')
      .toBe(emDias ? '/icons/fluent-emoji/calendar_3d.png' : '/icons/fluent-emoji/three_oclock_3d.png');
  }
});

test('O aviso de liberação vem acompanhado do círculo verde', async ({ page }) => {
  const indicador = page.getByTestId('ready-to-enter').first();
  await expect(indicador).toContainText('Você já pode entrar!');
  await expect(indicador.locator('img[src*="green_circle"]')).toBeVisible();
});
